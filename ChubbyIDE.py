...
class ChubbyIDE:
    ...
    KEYWORDS_TYPE = ["void", "bool", "char", "int", "long", "double", "string", "List"]
    ...
    def __init__(self, master: customtkinter.CTk):
        ...
        self.code_panel = customtkinter.CTkFrame(
            master, corner_radius=self.CORNER_RADIUS, fg_color=self.FRAME_COLOR
        )
        self.code_panel.grid(
        ...
        self.code_title = customtkinter.CTkLabel(
            self.code_panel,
            text="Chubby Code Editor",
            font=customtkinter.CTkFont(
                family=self.UI_FONT_FAMILY, size=self.UI_TITLE_FONT_SIZE, weight="bold"
            ),
        ...
        self.code_editor = customtkinter.CTkTextbox(
            self.code_panel,
            font=customtkinter.CTkFont(
                family=self.EDITOR_FONT_FAMILY, size=self.EDITOR_FONT_SIZE
            ),
        ...
        self.output_log_input_panel = customtkinter.CTkFrame(
            master, corner_radius=self.CORNER_RADIUS, fg_color=self.FRAME_COLOR
        )
        self.output_log_input_panel.grid(
        ...
        self.program_output = customtkinter.CTkTextbox(
            self.output_log_input_panel,
            font=customtkinter.CTkFont(
                family=self.LOG_FONT_FAMILY, size=self.LOG_FONT_SIZE
            ),
            state="disabled",
            corner_radius=self.CORNER_RADIUS,
            fg_color=self.TEXTBOX_COLOR,
        ...
        self.input_label = customtkinter.CTkLabel(
            self.output_log_input_panel,
            text="Program Input (one value per line):",
            font=customtkinter.CTkFont(
                family=self.UI_FONT_FAMILY, size=self.UI_TITLE_FONT_SIZE, weight="bold"
            ),
            text_color=self.LABEL_TEXT_COLOR,
        ...
        self.program_input_area = customtkinter.CTkTextbox(
            self.output_log_input_panel,
            font=customtkinter.CTkFont(
                family=self.LOG_FONT_FAMILY, size=self.INPUT_AREA_FONT_SIZE
            ),
            state="normal",
            corner_radius=self.CORNER_RADIUS,
            fg_color=self.TEXTBOX_COLOR,
        ...
        self.program_input_area.grid(
            row=3, column=0, padx=15, pady=(5, 10), sticky="nsew"
        )

        self.logs_label = customtkinter.CTkLabel(
            self.output_log_input_panel,
            text="Compilation Logs/Errors:",
            font=customtkinter.CTkFont(
                family=self.UI_FONT_FAMILY, size=self.UI_TITLE_FONT_SIZE, weight="bold"
            ),
            text_color=self.LABEL_TEXT_COLOR,
        ...
        self.compilation_logs.grid(
            row=5, column=0, padx=15, pady=(5, 15), sticky="nsew"
        )

        self.button_panel = customtkinter.CTkFrame(
            master, corner_radius=self.CORNER_RADIUS, fg_color="transparent"
        )
        ...
        self.load_file_button = customtkinter.CTkButton(
            self.button_panel,
            text="Load File",
            command=self.load_file_to_editor,
            font=customtkinter.CTkFont(
    ...
    def apply_highlighting(self):
        ...
        string_literal_combined_pattern_str = (
            f"(?:{string_main_pattern_str}|{char_main_pattern_str})"
        )
        ...
        keyword_control_pattern_str = (
            r"\b(?:" + "|".join(re.escape(kw) for kw in self.KEYWORDS_CONTROL) + r")\b"
        )
        keyword_type_pattern_str = (
            r"\b(?:" + "|".join(re.escape(kw) for kw in self.KEYWORDS_TYPE) + r")\b"
        )
        keyword_literal_pattern_str = (
            r"\b(?:" + "|".join(re.escape(kw) for kw in self.KEYWORDS_LITERAL) + r")\b"
        )
        keyword_modifier_pattern_str = (
            r"\b(?:" + "|".join(re.escape(kw) for kw in self.KEYWORDS_MODIFIER) + r")\b"
        )
        keyword_other_pattern_str = (
            r"\b(?:" + "|".join(re.escape(kw) for kw in self.KEYWORDS_OTHER) + r")\b"
        )
        operator_word_pattern_str = (
            r"\b(?:" + "|".join(re.escape(op) for op in self.OPERATORS_WORD) + r")\b"
        )
        ops_sym_multi_str = "|".join(
            re.escape(op) for op in self.OPERATORS_SYMBOLIC_MULTI
        )
        ops_sym_single_str = "|".join(
            re.escape(op) for op in self.OPERATORS_SYMBOLIC_SINGLE
        )
        operator_symbol_combined_pattern_str = (
            f"(?:(?:{ops_sym_multi_str})|(?:{ops_sym_single_str}))"
        )
    ...
    def update_text_widget(
        self, widget: customtkinter.CTkTextbox, content: str, text_color_override=None
    ):
        ...
        if text_color_override:
            ...
            try:
                ...
                print(
                    f"Warning: Could not configure tag {tag_name} for color {text_color_override}: {e}"
                )
            ...
            current_default_text_color = widget.cget("text_color")
            effective_default_color = (
                current_default_text_color
                if current_default_text_color
                else self.TEXTBOX_TEXT_COLOR
            )
    ...
    def load_file_to_editor(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Loading Error", f"An error occurred while loading the file: {e}"
            )
            self.update_text_widget(
    ...
    def compile_and_run(self):
        ...
        if not code:
            ...
            self.update_text_widget(
                self.compilation_logs, "No code entered.", self.WARNING_FG
            )
        ...
        self.update_text_widget(
            self.compilation_logs, "Starting compilation and execution...", self.INFO_FG
        )
        self.master.update_idletasks()

        success, stdout_output, error_logs = self.compiler.compile(
            code, ide_input=user_input_string
        )

        self.update_text_widget(
            self.program_output,
            stdout_output if stdout_output else "No output data.",
